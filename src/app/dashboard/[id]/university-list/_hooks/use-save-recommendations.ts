...
import { useAddRecommendationItem } from './use-add-recommendation-item';
...
export const useSaveRecommendations = ({ selectedItems, universityList }: UseSaveRecommendationsProps) => {
    ...
    const createMutation = useMutation({
        mutationFn: async () => {
            ...
            const items = selectedItems.map((admissionId, index) => {
                ...
                const suitabilityType = getSuitabilityType(university.recommendation_type || '재검토');

                const item = {
                    admission_id: admissionId,
                    rank: index + 1,
                    suitability_type: suitabilityType,
                    subject_suitability: university.recommendation_type || '적합',
                    non_subject_suitability: suitabilityType,
                    overall_evaluation: suitabilityType,
                    note: '추천 학교',

...
const CategoryEvaluationSection: FC<CategoryEvaluationSectionProps> = ({
    ...
    // 디바운스 자동 저장 기능
    useEffect(() => {
        if (editingCategory && editCategoryContent[editingCategory] !== undefined) {
            const currentCategory = categoryEvaluations?.find(cat => cat.main_category_id === editingCategory);
            const autoGeneratedContent = currentCategory?.auto_generated_texts?.join('\n');
            const originalContent = currentCategory?.final_content || autoGeneratedContent || '';

            if (editCategoryContent[editingCategory] !== originalContent) {
                // 이전 타이머가 있다면 클리어
                ...
                autoSaveTimeoutRef.current = setTimeout(() => {
                    if (onCategoryEvaluationUpdate) {
                        onCategoryEvaluationUpdate(editingCategory, editCategoryContent[editingCategory]);
                    }
                }, 5000);
            }
        }
    ...
    }, [editingCategory, editCategoryContent, categoryEvaluations, onCategoryEvaluationUpdate]);
